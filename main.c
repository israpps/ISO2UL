...
{
	printf("%d is the size of USBExtreme_game_entry_t\n", sizeof(USBExtreme_game_entry_t));
	...
	if (_read(fhin,copybuf,0x100000) != 0)
	{
		...
		printf("name variable was assigned [%s] as value\n", name);
		if (blksz == 2048)
		{
			ropen();
			do {
			...
			copybuf[0x2F]=part;
			copybuf[0x30]=0x12;
			if (temp[0]==0x64) copybuf[0x30]=0x14;
			copybuf[0x35]=0x08;
